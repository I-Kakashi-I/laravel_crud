    ...
    <div class="flex items-center justify-center">
        <div class="dark:bg-gray-800 p-5 rounded-xl bg-white w-full py-12" style="margin-top: -100px">
            ...
            <form class="w-full" action="{{route('users.show',$userData->id)}}" method="POST">
                @csrf
                <x-input_fild :value="$userData->name" id="name" type="text" name="name" label="Name"
                              placeholder="User name" disabled/>
                <x-input_fild :value="$userData->email" id="email" type="email" name="email" label="E-mail"
                              placeholder="user@example.com" disabled/>
                <x-input_fild :value="$userData->created_at" id="created_at" type="text" name="created_at"
                              label="E-Joined At" placeholder="" disabled/>
                <x-input_fild :value="$userData->updated_at" id="updated_at" type="text" name="updated_at"
