        ...
        <div class="dark:bg-gray-800 p-5 rounded-xl bg-white w-full py-12" style="margin-top: -100px">
            ...
            <form class="w-full" wire:submit.prevent="update" method="POST">
                ...
                <label for="role_select"
                       class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Roles</label>

                <x-roles-selector :allRoles="$allRoles" button="submit"/>
                <div class="flex items-center my-4">
                @foreach($allPermissions as $permission)
                        <input wire:model="selectedPermissions" id="permission_{{$permission->id}}" value="{{$permission->name}}" wire:key="{{$permission->id}}" type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                        <label for="permission_{{$permission->id}}" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300 mx-2">{{$permission->name}}</label>
                @endforeach
                </div>
