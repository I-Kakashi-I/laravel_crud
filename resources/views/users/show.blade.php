    ...
    <div class="max-w-4xl mx-auto  min-h-screen flex items-center justify-center">
        <div class="dark:bg-gray-800 p-5 rounded-xl bg-white w-full py-12" style="margin-top: -100px">
            ...
            <form class="w-full" action="{{route('users.show',$user->id)}}" method="POST">
                @csrf
                <x-input_fild :value="$user->name" id="name" type="text" name="name" label="Name" placeholder="User name" disabled/>
                <x-input_fild :value="$user->email" id="email" type="email" name="email" label="E-mail" placeholder="user@example.com" disabled/>
                <x-input_fild :value="$user->created_at" id="created_at" type="text" name="created_at" label="E-Joined At" placeholder="" disabled/>
                <x-input_fild :value="$user->updated_at" id="updated_at" type="text" name="updated_at" label="E-Updated At" placeholder="" disabled/>
